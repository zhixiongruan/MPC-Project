				...
				if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
					...
					if (s != "") {
						...
						if (event == "telemetry") {
							...
							// The cross track error is calculated by evaluating at polynomial at x, f(x).set x=0
							...
							double latency = 0.1;
							double Lf = 2.67;
							state(0) = 0.0 + v * latency;
							state(1) = 0.0;
							state(2) = 0.0 - v * delta / Lf * latency;
							state(3) = v + a * latency;
							state(4) = cte + v * sin(epsi) * latency;
							state(5) = epsi - v * delta / Lf * latency;
